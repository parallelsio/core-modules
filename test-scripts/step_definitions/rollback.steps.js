module.exports = function () {
  ...
  this.When(/^I create a new text bit on the canvas with content "([^"]*)"$/, function (content, callback) {
    ...
    self.browser.get('http://' + process.env.PARALLELS_DOMAIN).
      ...
      then(function () {
        return self.browser.findElement(self.webdriver.By.className("bit__editing")).sendKeys(content);
      }).
      then(function () {
        return self.browser.findElement(self.webdriver.By.className("bit__editing")).sendKeys(self.webdriver.Key.RETURN);
      }).
      then(callback).
