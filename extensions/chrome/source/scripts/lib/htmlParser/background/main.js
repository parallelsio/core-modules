...
define(['lib/htmlParser/background/observer', 'lib/htmlParser/background/bgcore', 'browser'],
  function (observer, core, browser) {
    ...
    function docInit(pageData, docData, maxIndex) {
      ...
      function pageInit() {
        ...
        processingPagesCount++;
        observer.publish('process-start', {
          data: {
            event: 'process-start',
            processStart: true,
    ...
    function docProgress(pageData, docData, index) {
      ...
      observer.publish('process-progress', {
        data: {
